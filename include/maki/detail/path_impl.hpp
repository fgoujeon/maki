...
namespace path_impl_detail
{
    ...
    {
        template<int Id, class PreviousResultNodeIpath>
        static constexpr auto call(
            const to_string_left_fold_result<PreviousResultNodeIpath>&
                previous_result)
        {
            using current_node_ipath_t =
                iseq_push_back_t<PreviousResultNodeIpath, Id>;
            ...
            {
                const auto state_pretty_name = detail::
                    pretty_name<MachineConfHolder, current_node_ipath_t>();
                const auto str =
                    previous_result.str + std::string{state_pretty_name} + "/";
                return to_string_left_fold_result<current_node_ipath_t>{str};
            }
        }
    };
