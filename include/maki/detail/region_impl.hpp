...
namespace region_detail
{
    ...
    namespace filter_state_mold_iseq_by_state_set_detail
    {
        ...
        struct for_state_set
        {
            ...
            struct matches
            {
                ...
                static constexpr bool value = contains(
                    impl_of(trans.source_state_mold),
                    target_state_mold);
            };
        };
    } // namespace filter_state_mold_iseq_by_state_set_detail
    ...
    using filter_state_mold_iseq_by_state_set_t = iseq_filter_t<
        StateMoldIseq,
        filter_state_mold_iseq_by_state_set_detail::for_state_set<
            MachineConfHolder,
            TransitionTablePath,
...
public:
    ...
    region_impl(const region<region_impl>* pitf, Machine& mach, Context& ctx):
        pitf_(pitf), states_(mix_uniform_construct, mach, ctx)
    {
    }
    ...
    [[nodiscard]] bool defers_event() const
    {
        ...
        {
            auto defers = false;
            with_active_state_mold<state_mold_iseq, state_defers_event<Event>>(
                *this,
                defers);
            return defers;
        }
    ...
    {
        execute_transition<state_mold_ids::null, state_mold_ids::ini, 0>(
            mach,
            ctx,
            event);
    }
    ...
    template<int TargetStateMoldId, class Machine, class Context, class Event>
    ...
    template<bool Dry, class Machine, class Context, class Event>
    bool
    process_event(const Machine& mach, Context& ctx, const Event& event) const
    {
        return process_event_2<Dry>(*this, mach, ctx, event);
    }
    ...
    static const auto& path()
    {
        static const auto value =
            maki::path{path_impl<MachineConfHolder, TransitionTablePath>{}};
        return value;
    }
...
private:
    ...
    {
        template<int StateMoldId, class Self, class Context, class Machine>
    ...
    process_event_2(Self& self, Machine& mach, Context& ctx, const Event& event)
    {
        ...
        if constexpr (must_try_executing_transitions &&
    ...
    {
        template<
            int ActiveStateMoldId,
            class Machine,
            class Context,
            class Event>
        static void
        call(region_impl& self, Machine& mach, Context& ctx, const Event& event)
        {
            self.execute_transition<
                ActiveStateMoldId,
                TargetStateMoldId,
                region_detail::null_action_index>(mach, ctx, event);
        }
    };
    ...
    */
    template<
        ...
        class Context,
        class Event>
    static bool try_executing_transitions(
        Self& self,
        Machine& mach,
        Context& ctx,
        const Event& event)
    ...
    struct try_executing_transition_2
    {
        ...
        call(Self& self, Machine& mach, Context& ctx, const Event& event)
        {
            ...
            {
                const auto& guard =
                    tuple_get<GuardIndex>(impl_of(trans_table)).grd;
    ...
    template<
        ...
        class Context,
        class Event>
    void execute_transition(Machine& mach, Context& ctx, const Event& event)
    {
        /*
        ...
        */
        if constexpr (TargetStateMoldId != state_mold_ids::internal)
        {
            impl_of(state_mold_id_to_state<SourceStateMoldId>())
                .exit(mach, ctx, event);
        }
        ...
        */
        if constexpr (TargetStateMoldId != state_mold_ids::internal)
        {
            ...
            impl_of(target_state).enter(mach, ctx, event);
        }
    ...
    struct process_event_in_active_state_2
    {
            ...
            bool& processed)
        {
            ...
            constexpr auto can_state_process_event = type_set_contains_v<
    ...
    struct is_active_state_mold_in_set_2
    {
        ...
        static void call([[maybe_unused]] bool& matches)
        {
            constexpr const auto& active_state_mold =
                machine_conf_tree::node_at_path_v<
                    MachineConfHolder,
                    iseq_push_back_t<TransitionTablePath, ActiveStateMoldId>>;
    ...
    {
        iseq_for_each_or<StateMoldIseq, with_active_state_mold_2<F>>(
            *this,
