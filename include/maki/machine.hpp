...
public:
    ...
    */
    ...
    bool check_event(const Event& event) const
    {
        ...
        submachine_.template call_internal_action<true>(*this, event, processed);
        return processed;
    }
...
private:
    ...
    void execute_one_operation(const Event& event)
    {
        ...
        {
            submachine_.call_entry_action(*this, event);
        }
        else if constexpr(Operation == detail::machine_operation::stop)
        {
            submachine_.call_exit_action(*this, event);
        }
        else
        {
            if constexpr(std::is_same_v<typename option_set_type::fallback_transition_action_tuple_type, detail::tuple<>>)
            {
                submachine_.call_internal_action(*this, event);
            }
            else
            {
                auto processed = false;
                submachine_.call_internal_action(*this, event, processed);
                if(!processed)
                {
